...
Spawns a sub-agent to complete a task. Unlike `CALL`, which invokes a subroutine in the same context, `DELEGATE` creates a separate agent with its own context boundary.
...
| `model` | no | Model tier for the sub-agent — same semantics as the global field |
...
The calling agent extracts the information described by `with` from its own context and passes it as the sub-agent's starting context. The sub-agent executes `task` and produces output matching `returns`, which flows back into the calling agent's context.

If `with` is omitted, the sub-agent operates from `task` alone, with no transferred context. If `returns` is omitted, the sub-agent's output is not explicitly captured — use this when the result is observable by other means (a written file, a sent message) rather than data needed by subsequent steps.

`DELEGATE` is distinct from `CALL`:

| | `CALL` | `DELEGATE` |
|---|---|---|
| Context | Shared — the sub-agent sees everything | Bounded — only what `with` describes |
| Routine | Explicit — `SUB` defines the steps | Implicit — the sub-agent determines its own steps |
| Return | None needed — context is already shared | Explicit via `returns` |
...
Available on any instruction, `SUB`, `DELEGATE`, or at root. The nearest scope takes precedence (inner overrides outer).
...
Available on the root process, any `SUB`, or any `DELEGATE`. The nearest scope takes precedence (inner overrides outer).
...
**Multi-agent.** When sections have different `model` values, distinct `role` definitions, or context makes it opportune, the agent may spawn sub-agents. No explicit declaration is needed — it is the expected behavior. For explicit delegation with defined context boundaries, use `DELEGATE`.
